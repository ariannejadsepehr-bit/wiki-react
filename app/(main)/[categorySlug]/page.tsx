...
import { MapPin, Filter, SortAsc, ChevronLeft, ChevronRight } from 'lucide-react';
...
function getPageInfo(slug: string) {
  ...
  if (city && catInfo) return { name: catInfo.name, description: catInfo.description, cityVariant: city };
...
const faqItems = [
  ...
  { q: 'آیا مشاوره اولیه رایگان است؟', a: 'بسیاری از وکلای ثبت‌شده در ویکی بهترین مشاوره اولیه رایگان ارائه می‌دهند.' },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const info = getPageInfo(categorySlug);
  ...
  const description = info.cityVariant
    ? `لیست بهترین ${info.name}های ${info.cityVariant} با امتیاز، نظرات و اطلاعات تماس.`
    : `لیست بهترین ${info.name}های ایران با امتیاز، نظرات و اطلاعات تماس.`;

  return {
    title, description,
    openGraph: { title, description, type: 'website' },
    alternates: { canonical: `https://wikibehtarin.com/${categorySlug}` },
  };
}
...
export default async function CategoryPage({ params, searchParams }: PageProps) {
  ...
  const breadcrumbs = [
    { label: 'خانه', href: '/' },
    ...(city ? [{ label: city, href: `/${categorySlug.split('-').pop()}` }] : []),
    { label: info.name, href: `/${categorySlug}` },
  ];

  const jsonLd = {
    '@context': 'https://schema.org', '@type': 'ItemList',
    name: pageTitle, url: `https://wikibehtarin.com/${categorySlug}`,
    itemListElement: mockBusinesses.map((biz, i) => ({
      '@type': 'ListItem', position: i + 1,
      item: {
        '@type': 'LocalBusiness', name: biz.name,
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pt-16">
        <section className="bg-white dark:bg-gray-950 border-b border-border">
          <div className="container-wiki py-10">
            ...
            <p className="text-muted-foreground text-base leading-7 max-w-2xl">
              با ویکی بهترین، بهترین {info.name}های {city || 'ایران'} را بر اساس امتیاز واقعی کاربران پیدا کنید.
            </p>
            <div className="flex flex-wrap items-center gap-3 mt-6">
              ...
              <div className="mr-auto text-sm text-muted-foreground">۴۸ نتیجه یافت شد</div>
            </div>
          </div>
        </section>
        ...
        <div className="container-wiki py-8">
          <div className="flex gap-6">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center justify-center gap-2 mt-10">
                ...
                {[1, 2, 3, '...', 8].map((p, i) => (
                  <button key={i} className={`w-9 h-9 rounded-xl text-sm font-medium transition-colors ${p === currentPage ? 'bg-blue-700 text-white' : 'border border-border hover:bg-accent'}`}>
          ...
          <div className="mt-12 bg-white dark:bg-gray-950 rounded-2xl border border-border p-6 lg:p-8">
            <h2 className="text-xl font-bold text-foreground mb-4">راهنمای انتخاب {info.name} {city && `در ${city}`}</h2>
            <div className="text-muted-foreground leading-7 space-y-3">
              <p>یافتن یک {info.name} مطمئن و متخصص یکی از مهم‌ترین تصمیماتی است که می‌توانید بگیرید. ویکی بهترین با جمع‌آوری اطلاعات کامل، نظرات واقعی کاربران و امتیازدهی شفاف، این فرآیند را برای شما آسان‌تر کرده است.</p>
            </div>
          </div>
          ...
          <div className="mt-6 bg-white dark:bg-gray-950 rounded-2xl border border-border p-6">
            ...
            <div className="flex flex-wrap gap-2">
              {['تهران', 'مشهد', 'اصفهان', 'شیراز', 'تبریز', 'کرج', 'اهواز', 'رشت'].map((c) => (
                <Link key={c} href={`/${categorySlug.replace(/-[a-z]+$/, '')}-${c === 'تهران' ? 'tehran' : c === 'مشهد' ? 'mashhad' : 'isfahan'}`}
